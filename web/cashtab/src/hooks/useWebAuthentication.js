...
const useWebAuthentication = () => {
   ...
   const loadAuthenticationConfigFromLocalStorage = async () => {
        ...
        try {
            ...
            console.error('Error is localforange.getItem("authenticatonConfig") in loadAuthenticationConfigFromLocalStorage() in useWebAuthentication()');
    ...
    const saveAuthenticationConfigToLocalStorage = () => {
        try {
            return localforage.setItem('authenticationConfig', {isAuthenticationRequired, userId, credentialId});
        } catch (err) {
    ...
    const authentication = {
        ...
        signIn: async () => {
            try {
                const assertion = await navigator.credentials.get({publicKey: publickKeyRequestOptions});
                if (assertion) {
    ...
    // (hardware failure, OS problems, the behaviour of some authenticators after serveral failed authentication attempts, etc)
